...
export default function Edit({ natures, decret }) {
    const { data, setData, edit, processing, errors } = useForm({
        date: decret.date, code_nat_prest: decret.code_nat_prest, taux: decret.taux
    });

    return (
        <AppLayout title="Modifier le décret RAS">
            <div className="max-w-lg bg-white rounded-xl shadow p-6">
                <form onSubmit={(e) => { e.preventDefault(); put(`/decret-ras/${decret.id}`); }} className="space-y-4">
                    <div>
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        <Link href="/decret-ras" className="bg-gray-200 text-gray-700 px-6 py-2 rounded-lg text-sm hover:bg-gray-300">Annuler</Link>
                    </div>
                </form>
            </div>
        </AppLayout>
